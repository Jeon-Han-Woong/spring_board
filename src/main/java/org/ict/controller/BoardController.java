...
public class BoardController {
	...
	// List<boardAttachBO>를 받아서, 해당 사진을 전부 폴더에서 삭제.
	private void deleteFiles(List<BoardAttachVO> attachList) {
		// 사진이 존재하지 않으면 아래쪽 로직을 실행할 필요가 없으므로
		// 바로 메서드를 중단하도록 처리.
		if(attachList == null || attachList.size() == 0) {
			return;
		}
		
		// 삭제할 데이터가 실제로 삭제된 파일과 일치하는지 디버깅.
		log.info(attachList);
		...
		attachList.forEach(attach -> {
			try {
				...
				// 업로드날짜(uploadPath) + UUID + 원본파일이름(fileName)
				Path file = Paths.get("c:\\upload_data\\temp\\" + attach.getUploadPath() + 
									"\\" + attach.getUuid() + "_" + attach.getFileName());
				
				// 파일 삭제시 Exception 없이 예외 경우도 넘어가는 삭제 메서드.
				Files.deleteIfExists(file);
				
				// 그림파일인 경우, 썸네일까지 추가로 삭제
				// 파일 타입이 이미지인 경우만 추가로 삭제
